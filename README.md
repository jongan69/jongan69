# Jonathan Gan

I build practical AI, automation, trading, and media tools. Most of my GitHub is an archive of experiments, hackathon projects, and prototypes; the projects below are the ones I currently consider most representative.

## Current Focus

- Agent tooling and MCP servers
- AI-assisted video and content workflows
- Mobile apps with Expo and React Native
- Trading, market data, and automation systems
- Small products that turn messy workflows into usable tools

## Selected Projects

- [shopping-deals-mcp-server](https://github.com/jongan69/shopping-deals-mcp-server) - MCP server for finding marketplace deals across eBay, Amazon, Craigslist, and OfferUp.
- [ClipCaptionAI](https://github.com/jongan69/ClipCaptionAI) - AI video editing toolkit for shorts, masked captions, B-roll planning, and Remotion workflows.
- [PrepAI](https://github.com/jongan69/PrepAI) - Mobile meal-planning app that uses computer vision and AI to turn available ingredients into plans.
- [odysseus-expo](https://github.com/jongan69/odysseus-expo) - Expo mobile companion for the Odysseus AI workspace.
- [soltrendio](https://github.com/jongan69/soltrendio) - Solana wallet analysis and trend tooling.
- [trading-api](https://github.com/jongan69/trading-api) - Rust market-data API.

## Background

I have worked across React, React Native, TypeScript, Python, Rust, automation, and applied AI. My earlier work includes client delivery engineering, blockchain research, internal tool building, mobile prototypes, and hackathon products.

## Links

- [Expo](https://expo.dev/@jongan69)
- [Devpost](https://devpost.com/jongan69)
- [LeetCode](https://leetcode.com/jongan69/)
